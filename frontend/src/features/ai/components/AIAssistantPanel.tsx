...
const quickActions: { intent: AIIntent; label: string; message: string }[] = [
  { intent: 'generate_wbs', label: 'Gerar EAP', message: 'Crie uma estrutura analitica detalhada para este projeto.' },
  { intent: 'suggest_next_steps', label: 'Proximos passos', message: 'Quais atividades devo priorizar agora?' },
...
export const AIAssistantPanel = () => {
  ...
  const sendMessage = async (intent: AIIntent, message: string) => {
    ...
    const response = await aiClient.act({ projectId: selectedProjectId, intent, message: content })
    ...
    <aside className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white shadow-soft">
      <div className="border-b border-slate-200 px-5 py-4">
        ...
        <p className="mt-1 text-xs text-slate-500">
          O agente utiliza o contexto do projeto selecionado para gerar tarefas, responder duvidas e atualizar status.
        </p>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {quickActions.map((action) => (
            <button
              ...
              onClick={() => sendMessage(action.intent, action.message)}
